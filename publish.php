<?php
    ...
    if (!empty($_POST['name'])) {
        ...
        // 画品简介
        $desc = mysql_real_escape_string(trim($_POST['des']));
        // 画品详情
        ...
        $pic = imageUpload($file);

        // 建议大家做商品名称唯一性验证处理
        $sql = "SELECT * FROM im_goods WHERE name = '{$name}'";
        $obj = mysql_query($sql);
        $result = mysql_fetch_assoc($obj);
        print_r($result);
        if (isset($result['name']) && $result['name'] == $name) {
            msg(2, '该商品名称已经存在'); exit;
        }

        $sql = "INSERT INTO im_goods(`name`, `price`, `desc`, `content`, `pic`, `user_id`, `create_time`, `update_time`, `view`) 
                VALUES('{$name}', '{$price}', '{$desc}', '{$content}', '{$pic}', '$user_id', '{$now}', '{$now}', 0)";
        echo $sql;
